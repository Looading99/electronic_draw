...
void setup() {
  ...
  uint8_t mask = EEPROM.read(0), ui;
  for (ui = 0; ui < NUM_OF_STUDENTS; ui++) {
    if (EEPROM.read(ui) >> 6 != mask) called_last_modified = ui;
  }
}

void loop() {
  if ((millis()-last_use_sec)>30*1000) {
  ...
  id = get_random_id();
  EEPROM.write(called_last_modified, id | ((EEPROM.read(called_last_modified - 1) >> 6) + 1) % 0b11);
  for (ui = 0; ui < 2; ui++) {
    show_id(id);
    delay(200);
